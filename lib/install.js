...
const KiteProcessError = require('kite-connector/lib/kite-process-error');
...
module.exports = class Install {
  ...
  */
  constructor(steps, state = {}, options = {}) {
    ...
    this.flow.onDidFailStep(({step, error}) => {
      if (error instanceof KiteProcessError) {
        rollbar.error(error, step);
      }
    });
  }
