...
class InstallElement extends HTMLElement {
  ...
  updateView() {
    if (this.install.flow.currentStep && this.install.flow.currentStep.view) {
      if (this.children.length) {
        [].slice.call(this.children).forEach(n => this.removeChild(n));
      }
      this.appendChild(this.install.flow.currentStep.view);
      this.install.flow.currentStep.view.setInstall(this.install);
    }
  }
}
