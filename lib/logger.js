...
const Logger = {
  ...
  logResponse(resp) {
    let data = '';
    this.verbose('response received', resp);

    resp.on('error', err =>
      this.verbose('error occured', err, resp));
    resp.on('aborted', (...args) =>
      this.verbose('response aborted', ...args));
    resp.on('close', (...args) =>
      this.verbose('response closed', ...args));
    resp.on('data', (chunk) => {
      data += chunk;
      this.silly('data received', chunk);
    });
    resp.on('end', () => this.verbose('response ended', data));
  },
  log(level, ...msgs) {
