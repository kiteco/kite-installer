...
module.exports = class Flow extends BaseStep {
  ...
  onDidGetStepData(listener) {
    return this.emitter.on('did-get-step-data', listener);
  }
  ...
  executeStep(step) {
    ...
    .catch((err) => {
      if (step.retryStep) {
        this.emitter.emit('did-get-step-data', {error: err.message});
    ...
    .then((data) => {
      if (data && data.step) {
        this.emitter.emit('did-get-step-data', data.data);
        return this.executeStep(data.step);
      } else {
        this.emitter.emit('did-get-step-data', data);
