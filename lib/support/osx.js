...
const OSXSupport = {
  ...
  get installPath() {
    return String(child_process.execSync('mdfind kMDItemCFBundleIdentifier = "com.kite.Kite"')).trim();
  },

  get entrepriseInstallPath() {
    return String(child_process.execSync('mdfind kMDItemCFBundleIdentifier = "entreprise.kite.Kite"')).trim();
  },
  ...
  hasBothKiteInstalled() {
    return Promise.all([
      ...
      this.isKiteEntrepriseInstalled(),
    ]);
  },

  isKiteEntrepriseInstalled() {
    return utils.spawnPromise('mdfind', ['kMDItemCFBundleIdentifier = "entreprise.kite.Kite"'])
    .then(res => {
      if (!res || res.trim() === '') {
        throw new KiteError('bad_state', STATES.UNINSTALLED);
      }
  ...
  isKiteEntrepriseRunning() {
    ...
    .then(stdout => {
      const procs = stdout.split('\n');
      if (!procs.some(s => /^KiteEntreprise$/.test(s))) {
        throw new KiteError('bad_state', STATES.INSTALLED);
      }
    });
  },
  ...
  runKiteEntreprise() {
    ...
    .then(() =>
      utils.spawnPromise('open', ['-a', this.entrepriseInstallPath]));
  },
};
