...
const OSXSupport = {
  ...
  get allInstallPath() {
    return String(child_process.execSync('mdfind kMDItemCFBundleIdentifier = "com.kite.Kite"')).trim().split('\n');
  },
  ...
  get allEnterpriseInstallPath() {
    return String(child_process.execSync('mdfind kMDItemCFBundleIdentifier = "enterprise.kite.Kite"')).trim().split('\n');
  },
  ...
  hasManyKiteInstallation() {
    return this.allInstallPath.length > 0;
  },

  hasManyKiteEnterpriseInstallation() {
    return this.allEnterpriseInstallPath.length > 0;
  },
