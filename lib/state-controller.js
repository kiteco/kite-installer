...
const StateController = {
  ...
  isKiteEntrepriseInstalled() {
    Logger.verbose('isKiteEntrepriseInstalled called');
    return this.isKiteSupported()
    .then(() => this.support.isKiteEntrepriseInstalled());
  },
  ...
  isKiteEntrepriseRunning() {
    Logger.verbose('isKiteEntrepriseRunning called');
    return this.isKiteEntrepriseInstalled()
    .then(() => this.support.isKiteEntrepriseRunning());
  },

  canRunKiteEntreprise() {
    Logger.verbose('canRunKiteEntreprise called');
    return this.isKiteEntrepriseInstalled()
    .then(() =>
      utils.reversePromise(this.isKiteEntrepriseRunning(),
        new KiteError('bad_state', STATES.RUNNING)));
  },

  runKiteEntreprise() {
    Logger.verbose('runKiteEntreprise called');
    return this.canRunKiteEntreprise()
    .then(() => this.support.runKiteEntreprise())
    .catch((err) => {
      if (err.data !== STATES.RUNNING) {
        throw err;
      }
  ...
  runKiteEntrepriseAndWait(attempts, interval) {
    Logger.verbose('runKiteEntrepriseAndWait called', attempts, interval);
    return this.runKiteEntreprise().then(() => this.waitForKite(attempts, interval));
  },
