...
var StateController = {
  ...
  writeKite: function(stream, opts={}) {
    return new Promise((resolve, reject) => {
      ...
      var apm = () => {
        this.deactivateAtomPackage();
        var proc = this.installAtomPackage();
        if (!proc) {
          if (typeof(opts.onPackageInstall) === 'function') {
            opts.onPackageInstall();
          }
          resolve();
          return;
        }
        proc.on('close', (code) => {
          ...
          this.activateAtomPackage().then(() => {
            this.enableAtomPackage();
          });
          resolve();
        });
      };
  ...
  runKite: function() {
    return new Promise((resolve, reject) => {
      this.canRunKite().then(() => {
        ...
        child_process.spawnSync('defaults' ['write', 'com.kite.Kite', 'shouldReopenSidebar', '0']);

        var p = child_process.spawnSync('open', ['-a', this.KITE_APP_PATH.installed], {
          encoding: 'utf8',
        });
        console.log(`runKite stdout: ${ p.stdout }`);
        console.log(`runKite stderr: ${ p.stderr }`);
        resolve();
      }, (err) => {
        reject(err);
      });
  ...
  get apm() {
    var path = '/Applications/Atom.app' +
  ...
  isAtomPackageInstalled: function() {
    return new Promise((resolve, reject) => {
      if (!this.apm) {
        reject();
        return;
  ...
  enableAtomPackage: function() {
    return atom ? atom.packages.enablePackage('kite') : null;
  },

  disableAtomPackage: function() {
