...
const StateController = {
  ...
  pathInWhitelist(path) {
    return !path
      ...
      : this.client.request({
        path: `/clientapi/permissions/authorized?filename=${path}&localtoken=${this.client.LOCAL_TOKEN}`,
        method: 'GET',
      })
      .catch(err => { throw new KiteError('http_error', err); })
      .then(resp => {
  ...
  canWhitelistPath(path) {
    ...
    .then(() =>
      this.client.request({
        path: `/clientapi/projectdir?filename=${path}&localtoken=${this.client.LOCAL_TOKEN}`,
        method: 'GET',
      })
      .catch(err => { throw new KiteError('http_error', err); })
      .then(resp => {
