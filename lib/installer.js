...
var Installer = class {
  ...
  install() {
    ...
    this.installPromise.then(() => {
      StateController.waitForKite(120, Installer.INTERVAL).catch((err) => {
        if (typeof(this.dismissKite) === 'function') {
          this.dismissKite();
          Tracker.trackEvent("account creation aborted", { error: err });
          console.error("account creation aborted:", err);
        }
      });
    }, (err) => {
  ...
  parseUserEmail() {
    try {
      ...
      console.error("error parsing gitconfig:", err);
    }
  }
};
